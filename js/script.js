const Apikey = " 5e49336666f94335821aff77dfce6c75";
...
function getListTeams(){
    title.innerHTML="Daftar Tim Primer Liga Inggris";
    fetch(teamEndPoin,fetchHeader)
        ...
        resJson.teams.forEach(team=>{
            ...
            <li class="collection-item avatar">
              ...
              <a href="#info" data-id="${team.id}" class="secondary-content"><i class="material-icons" data-id="${team.id}">info</i></a>
            </li>
            `
        });
...
function showTeamInfo(){
    ...
    fetch(teamEndPoin, fetchHeader)
        ...
        .then(resJson => {
            ...
            console.log(resJson.teams);
            let sad = "";
            resJson.teams.forEach(info => {
                   sad =
                   `
                   <div class="card horizontal">                
                     <p> Nama: ${info.name} <br>
                         Alamat: ${info.address} <br>
            ...
            contents.innerHTML = sad
        })
}


function getListStandings() {
    title.innerHTML="Klasmen Sementara Liga Primer Inggris";
...
function getListMatches() {
    title.innerHTML="Jadwal Tim Liga Primer Liga Inggris";
    fetch(matchEndPoin,fetchHeader)
       ...
       resJson.matches.forEach(match => {
           let d = new Date(match.utcDate).toLocaleString("id");
           let scoreHomeTeam = (match.score.fullTime.homeTeam      == null ? 0 : match.score.fullTime.homeTeam);
           let scoreAwayHomeTeam = (match.score.fullTime.awayTeam  == null ? 0 : match.score.fullTime.awayTeam);
             matchs += `
             <tr>  
                <td style="padding-left:20px;">${i}.</td>
                <td>${match.homeTeam.name} vs ${match.awayTeam.name}</td>
                <td>${d}}</td>
                <td>${scoreHomeTeam}:${scoreAwayHomeTeam}</td>
            </tr>
              `;
              i++;
        });
